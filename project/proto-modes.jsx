      ...
      <div className="atmenu-list">
        {n ? m.results.map((id, i) => (
          <button key={id} className={"atmenu-row" + (i === m.sel ? " on" : "")}
            ...
            <span className="atmenu-txt">{truncate(notes[id].text || "", 64)}</span>
          </button>
...
function NeighborCard({ id, rel, notes, onOpen, ghost }) {
  ...
  return (
    <div className={"ncard" + (ghost ? " ghost" : "")} onClick={() => onOpen(id)}>
      ...
      <div className="ntext">{truncate(note.text, 88)}</div>
    </div>
  );
}
  ...
  return (
    <div className="kil-wrap">
      <div className="kil">
        ...
        <span className="kil-text">Luhm sieht eine Nähe zu <button className="kil-to" onClick={() => onOpen(id)} title={truncate(notes[id].text, 88)}>{id}</button></span>
    ...
    <div className="splitcol">
      <div className="reading lesen-reading" style={{ position: "relative", flex: 1, display: "flex", flexDirection: "column", justifyContent: "flex-start", overflowY: "auto", padding: "clamp(40px, 7vh, 84px) clamp(40px, 7vw, 96px)", minWidth: 0 }}>
        ...
        <div style={{ width: "100%", maxWidth: 660, margin: "0 auto" }}>
          ...
          {g.ki.length > 0 && (
            <div className={"ki-lean" + (belegeIds.length === 0 ? " sep" : "")}>
              ...
              <div className="kil-foot">{(feedbackStats.accepted + feedbackStats.rejected) > 0
                ...
                : "Ein Vorschlag, kein Befehl. Übernimm ihn oder verwirf ihn — Luhm lernt daraus."}</div>
            </div>
          )}

          <div style={{ display: "flex", alignItems: "center", gap: 4, marginTop: 36, flexWrap: "wrap" }}>
            ...
            {onAddToHub && <button className="act" onClick={onAddToHub}><Ic.layers /> Als Einstieg</button>}
            {belegeIds.length === 0 && <button className="act" onClick={onAttachBeleg}><Ic.quote /> Beleg</button>}
          </div>
    ...
    <div className="replytrail">
      ...
      {replies.map((r) => (
        <button key={r.id} className="replytrail-item" onClick={() => onOpen(r.id)}>
          ...
          <span className="replytrail-txt">{truncate(notes[r.id].text, 130)}</span>
        </button>
      ))}
    </div>
  );
...
function ContextRow({ whisper, replies, notes, onOpen }) {
  ...
  return (
    <div className="ctxrow">
      ...
      {replies.map((r) => (
        <button key={r.id} className="ctxrow-chip" onClick={() => onOpen(r.id)} title={truncate(notes[r.id].text, 100)}>
  ...
  return (
    <div className="gwrap" ref={wrapRef}>
      ...
      {items.map((p) => (
        <div key={p.id} className={"gcard gcard-" + p.zone + (p.rel === "ki" ? " is-ki" : "")} style={{ left: p.x, top: p.y }} onClick={() => onOpen(p.id)}>
          <div className={"ncard" + (p.rel === "ki" ? " ghost" : "")}>
            ...
            <div className="ntext">{truncate(notes[p.id].text, 58)}</div>
          </div>
        </div>
      ))}
      ...
      <svg width={sz.w} height={sz.h} style={{ position: "absolute", inset: 0, zIndex: 4, pointerEvents: "none" }}>
        ...
        {zoneLabel(g.folge.length, cx, yB + cardHH + 22, "FOLGEZETTEL")}
        {zoneLabel(g.verzweig.length, xR, minY("right") - cardHH - 12, "VERZWEIGUNG")}
        {zoneLabel(leftItems.length, xL, minY("left") - cardHH - 12, "ANDERE INDIZES")}
      </svg>
      ...
      <div className="gcenter" style={{ left: cx, top: cy }}>
        <div className="inner" ref={centerRef}>
          ...
          <p className="zettel-body" style={{ fontSize: 16.5, lineHeight: 1.5, margin: "14px 0 0", textWrap: "pretty" }}>{truncate(note.text, 180)}</p>
